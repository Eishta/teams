...
import { createTeam, deleteTeam } from '../actions/actions'
          ...
          <div className={`${classOne}`} id="navbarSupportedContent">
            <ul className="navbar-nav">
              ...
              {teams.map((team, index) => <li key={index} className="nav-item">
                ...
                <Redirect to='/'><button onClick={()=>this.deleteTeamByName(team.name)}>X</button></Redirect>
              </li>)}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    deleteTeam: (name)=> dispatch(deleteTeam(name))
  }
}
